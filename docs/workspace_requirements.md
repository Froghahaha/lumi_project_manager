...
## 0604
### 超级管理员工作台
1. 预警、逾期显示归入正常运行中，但是倒计时字颜色不一样
2. 验收工序取消，并入调机
3. 超级管理员视角下，项目卡片抬头增加项目经理和销售的姓名显示
4. 项目卡片优化尾款工序和进度条展示
5. 项目卡片增加显示工序完成时间点
6. 项目卡片排序按照项目生效时间排序
7. 筛选逻辑 总筛选+二级筛选
    - 一级是项目总状态筛选：正常运行中/预警/逾期/已完成
    - 二级是按工序类型筛选

### 项目看板组件
1. 增加筛选定制
2. 增加项目卡片搜索框(搜索结果仍然进入筛选)
    - 按项目名称搜索
    - 按人员名称搜索
3. 卡片hover时，要进行凸显

### 工序状态管理
1. 机械涉及工序，一旦被指派了执行人，就变成设计中
2. 取消验收工序，在调机阶段设置 安调中/安调完成/验收完成 这几个阶段
...
### 销售助理界面
1. 设备的描述输入框，提示信息更改为型号

### 数据结构
项目的设备类型增加：桁架、视觉桁架、工业机器人、协作机器人、智能上下料打磨机床、输送带、板链机、顶升机、提升机、非标设备